...
void cpu_histgo()
	...
	for(i = 0; i < height; ++i)
		...
		for(j = 0; j < width; ++j)
			...
			t = max(max(r,g),max(b,a));

			hostBin[t]++;
			//printf("hostbin %d=%d\n", data[i * width + j], hostBin[data[i * width + j]]);
			}
		}
	//直接统计R,G,B, A一个大于等于5的像素
	    int counter = 0;
		for(i = 0; i < height; ++i)
			{
			for(j = 0; j < width; ++j)
		...
		printf("counter=%d\n",counter);
		printf("caculate counter=%d\n",width*height - hostBin[0] - hostBin[1] - hostBin[2] - hostBin[3] - hostBin[4]);
	}
...
int main(int argc, char* argv[])
	...
	// 分配内存
	data = (cl_uchar*)malloc(width * height * 4 * sizeof(cl_uchar));
	if(!data)
		{
		printf("分配内存失败\n");
		return 0;
		}
	memset(data, 0,width * height * 4* sizeof(cl_uchar));
	...
	dataBuf = clCreateBuffer(
		context, 
		CL_MEM_READ_ONLY,
		sizeof(cl_uchar) * width  * height * 4,
		NULL, 
		0);
