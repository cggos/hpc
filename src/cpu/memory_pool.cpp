...
FixedBlockPool::FixedBlockPool(std::size_t block_size, std::size_t block_count)
    : block_size_(align_up(block_size, alignof(std::max_align_t))), block_count_(block_count) {
  if (block_size == 0 || block_count == 0) {
    throw std::invalid_argument("block size and block count must be positive");
  }

  storage_ = ::operator new (block_size_* block_count_, std::align_val_t{alignof(std::max_align_t)});
...
FixedBlockPool::~FixedBlockPool() { ::operator delete (storage_, std::align_val_t{alignof(std::max_align_t)}); }
